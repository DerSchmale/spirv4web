...
in highp vec2 uv1;
in highp vec2 uv0;
layout(location = 0) out highp vec4 hx_fragColor;
...
HX_FragmentData hx_fragmentData()
{
    ...
    if (USE_MAP_colorMap)
    {
        ...
        data.color *= texture(colorMap, (vec2(_154.x ? uv1.x : uv0.x, _154.y ? uv1.y : uv0.y) + colorMapOffsetUV) * colorMapScaleUV);
    }
    if (USE_MAP_maskMap)
    {
        bvec2 _177 = bvec2(_174);
        data.color.w *= texture(maskMap, (vec2(_177.x ? uv1.x : uv0.x, _177.y ? uv1.y : uv0.y) + maskMapOffsetUV) * maskMapScaleUV).x;
    }
    ...
    if (_222)
    {
        ...
        highp vec4 specSample = texture(specularMap, (vec2(_232.x ? uv1.x : uv0.x, _232.y ? uv1.y : uv0.y) + specularMapOffsetUV) * specularMapScaleUV);
        if (_241)
        {
    ...
    if (USE_MAP_emissiveMap)
    {
        ...
        data.emissive *= texture(emissiveMap, (vec2(_285.x ? uv1.x : uv0.x, _285.y ? uv1.y : uv0.y) + emissiveMapOffsetUV) * emissiveMapScaleUV).xyz;
    }
    if (USE_MAP_occlusionMap)
    {
        bvec2 _308 = bvec2(_305);
        data.occlusion *= texture(occlusionMap, (vec2(_308.x ? uv1.x : uv0.x, _308.y ? uv1.y : uv0.y) + occlusionMapOffsetUV) * occlusionMapScaleUV).x;
    }
    if (USE_MAP_lightMap)
    {
        bvec2 _332 = bvec2(_329);
        highp vec4 lightMapVal = texture(lightMap, (vec2(_332.x ? uv1.x : uv0.x, _332.y ? uv1.y : uv0.y) + lightMapOffsetUV) * lightMapScaleUV);
...
void main()
{
    ...
    highp vec4 _366 = hx_fragColor;
    highp vec3 _368 = _366.xyz + _365;
    hx_fragColor.x = _368.x;
    hx_fragColor.y = _368.y;
    hx_fragColor.z = _368.z;
    highp vec4 param_2 = hx_fragColor;
    hx_fragColor = hx_linearToScreen(param_2);
}
