...
layout(std140) uniform hx_entity
{
...
layout(std140) uniform hx_skinning
{
    vec4 hx_skinningMatrices[_176];
} _180;

layout(std140) uniform hx_morphing
{
    vec4 hx_morphWeights[2];
} _276;

layout(std140) uniform hx_camera
{
...
layout(std140) uniform hx_const
{
    vec2 hx_poissonDisk[32];
} _477;

layout(std140) uniform hx_scene
{
...
layout(location = 7) in vec4 hx_instanceMatrix0;
layout(location = 8) in vec4 hx_instanceMatrix1;
layout(location = 9) in vec4 hx_instanceMatrix2;
layout(location = 0) in vec4 hx_position;
out vec2 uv0;
layout(location = 3) in vec2 hx_texCoord;
out vec2 uv1;
layout(location = 4) in vec2 hx_texCoord1;
layout(location = 11) in vec3 hx_morph0;
layout(location = 12) in vec3 hx_morph1;
layout(location = 13) in vec3 hx_morph2;
layout(location = 14) in vec3 hx_morph3;
layout(location = 15) in vec3 hx_morph4;
layout(location = 16) in vec3 hx_morph5;
layout(location = 17) in vec3 hx_morph6;
layout(location = 18) in vec3 hx_morph7;
layout(location = 6) in vec4 hx_jointWeights;
layout(location = 5) in vec4 hx_jointIndices;
layout(location = 1) in vec3 hx_normal;
layout(location = 2) in vec4 hx_tangent;
layout(location = 10) in float hx_instanceID;
out vec2 uv2;
    ...
    if (HX_USE_SKINNING)
    {
        if (HX_USE_SKINNING_TEXTURE)
        {
            for (int i = 0; i < 4; i++)
            {
                ...
                mat[0] += (textureLod(hx_skinningTexture, vec2(index, 0.0), 0.0) * weights[i]);
                mat[1] += (textureLod(hx_skinningTexture, vec2(index + HX_SKELETON_PIXEL_SIZE, 0.0), 0.0) * weights[i]);
                mat[2] += (textureLod(hx_skinningTexture, vec2(index + (2.0 * HX_SKELETON_PIXEL_SIZE), 0.0), 0.0) * weights[i]);
            }
        }
        else
        {
